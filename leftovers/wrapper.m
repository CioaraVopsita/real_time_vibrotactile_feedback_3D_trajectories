...
%% Repeat until break
    while getanswer('...ready for next trial (''N'' to exit)?',1),
        ...
        %differentiation
        data = zeros(ceil((trial_duration + min_random_duration + max_random_duration)/sample_interval)+1,size(markers,2)*6+2+1);
        go = 0;
        ...
        sampletime=0;
        abort = false;
        %!!!!! start sample from 2 to allow first ROW to be zeros
        sample = 2;
        %flush the buffer
        h.flush;
